...
3. เปรียบเทียบแหล่งตาม src/config/news-sources.ts:
   - **ไอที/เทคโนโลยี: เช็ก https://www.blognone.com/ ก่อนเสมอ** (แหล่งหลัก) — ถ้ามีเรื่องเด่นของวันนั้น ให้เพิ่มอย่างน้อย 1 เรื่องจาก Blognone
   - หมวดอื่น: https://www.thaipbs.or.th/news/archive/YYYY-MM-DD และแหล่งใน config
4. เพิ่มเฉพาะเรื่องใหม่ที่ยืนยันแล้ว (+2–4 เรื่อง/รอบ) — ห้ามซ้ำ slug/มุมเดิม — **`publishedAt` ต้องตรงวันที่ต้นทางเผยแพร่ ห้ามย้ายข่าวเก่ามาใส่วันนี้**
...
7. เรื่องใหม่: fetch-image → **`npm run build`** (auto push หลัง build ผ่าน — ไม่ต้องถาม user)
8. สรุปให้ user: เรื่องที่เพิ่ม/อัปเดต, จำนวนข่าววันนั้น, มี IT จาก Blognone หรือไม่, build/push สำเร็จหรือไม่
