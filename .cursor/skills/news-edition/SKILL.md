...
2. เปรียบเทียบกับแหล่งตาม `src/config/news-sources.ts` — **หมวดไอที/เทคโนโลยี: เช็ก [Blognone](https://www.blognone.com/) ก่อนเสมอ** (แหล่งหลัก) แล้วค่อยแหล่งสำรอง; หมวดอื่นเทียบ [ไทยพีบีเอส archive วันนั้น](https://www.thaipbs.or.th/news/archive/YYYY-MM-DD) และแหล่งใน config
3. **เพิ่มเรื่องใหม่** เมื่อ: เหตุการณ์ใหม่ / กระแสวันนั้นชัด — ใช้ workflow `news-reader → news-summarizer → news-citation → news-writer`
4. **อัปเดตไฟล์เดิม** เมื่อ: เรื่องเดิมมีพัฒนาการ (ไม่สร้าง slug ซ้ำมุมเดิม)
5. **ไม่เพิ่ม** เมื่อ: ข่าวลือ / ซ้ำเรื่องเดิม / รายละเอียดเล็กน้อย / **ต้นทางเผยแพร่ก่อนวันฉบับ** (ยกเว้นอัปเดตเรื่องค้างที่มีพัฒนาการใหม่วันนั้น)
6. ฉบับควรมีข่าว **ไอที/เทคโนโลยี จาก Blognone** เมื่อ Blognone มีเรื่องเด่นของวันนั้น — อย่าข้ามหมวดนี้เพราะดึงแต่ไทยพีบีเอส
7. `npm run fetch-image` สำหรับเรื่องใหม่
8. **`npm run build`** — generate-audio (prebuild) → build → **postbuild auto commit + push**

## Publish ขึ้น Git (อัตโนมัติ)

หลังเพิ่ม/แก้ข่าวสำเร็จ รัน **`npm run build`** ครั้งเดียว — build ผ่านแล้ว push เองทันที **ไม่ต้องถาม user**

```bash
npm run build                     # build + auto push (ค่าเริ่มต้นหลังเพิ่มข่าว)
npm run build:only                # build อย่างเดียว ไม่ push
npm run publish-edition           # เท่ากับ npm run build
npm run publish-news -- --dry-run # ดู preview อย่างเดียว
npm run publish-news -- --no-push # commit อย่างเดียว ไม่ push
```

`npm run build:only` **ไม่ push** — ใช้เมื่อ dev/test

## เกณฑ์ครบฉบับ

- **`publishedAt` = วันที่ต้นทางเผยแพร่จริง** (ตรวจจาก byline/archive ของ sourceUrl) — ห้ามเติมหมวดว่างด้วยข่าวเก่า
- ครบ 7 หมวดที่มีข่าวในวันนั้น (ถ้ามีแหล่งจริงของวันนั้น)
